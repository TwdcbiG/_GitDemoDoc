...
- nvm安装后，setting.text中的root路径
  ...
  这里是通过nvm安装的nodejs版本文件的存放路径。可以随意修改。但必须修改为已存在路径。
...
这也是有一些回答说遇到这个问题删除文件夹中nodejs文件的原因。因为nvm安装时symlink这里默认的就是C:\Program Files\nodejs文件夹。
...
使main.js文件中new Vue的el属性名与index.heml中的 div id相同，即可完成绑定。
...
template中的内容将会完全替代index.html中el绑定的内容。即使其中的内容`<TYGH>dfasd</TYGH>`什么也不代表。
